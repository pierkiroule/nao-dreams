...
test("chaque emoji disponible peut nourrir un rêve",()=>{for(let index=0;index<EMOJIS.length;index++){const sequence=[EMOJIS[index],EMOJIS[(index+1)%EMOJIS.length],EMOJIS[(index+2)%EMOJIS.length]],dream=generateProjectiveDream(sequence);assert.ok(dream.title);assert.ok(dream.parts.join(" ").length>120);assert.equal(dream.gaps.length,4)}});
